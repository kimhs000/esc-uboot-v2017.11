...
static iomux_v3_cfg_t const usdhc3_pads[] = {
	...
	IOMUX_PADS(PAD_SD2_CLK__GPIO1_IO10 | MUX_PAD_CTRL(NO_PAD_CTRL)), /* CD */
};
...
//	IOMUX_PADS(PAD_EIM_D22__USB_OTG_PWR | MUX_PAD_CTRL(NO_PAD_CTRL)),
	...
	IOMUX_PADS(PAD_EIM_D21__USB_OTG_OC    | MUX_PAD_CTRL(NO_PAD_CTRL)), //Over current

};

static iomux_v3_cfg_t const usb_hc1_pads[] = {
//	IOMUX_PADS(PAD_ENET_TXD1__GPIO1_IO29 | MUX_PAD_CTRL(NO_PAD_CTRL)),
	IOMUX_PADS(PAD_EIM_D30__USB_H1_OC | MUX_PAD_CTRL(NO_PAD_CTRL)),
...
int power_init_board(void)
{
	...
	/* Increase SW from 3V to 3.3V */
	pmic_reg_read(p, PFUZE100_SW2VOL, &reg);
	//printf("PFUZE100_SW2VOL regster : 0x%x\n", reg);
	reg &= ~SW2_MODE_MASK;
	reg |= SW2x_3_300V;
	//printf("PFUZE100_SW2VOL regster : 0x%x\n", reg);
...
int checkboard(void)
{
	puts("Board: Embedded SmartCamera Board\n");
	return 0;
}
